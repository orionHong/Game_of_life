...
public class Life extends JPanel implements ActionListener, MouseListener, MouseMotionListener {
    ...
    private void doFrame() { // Compute the new state of the Life board.
        boolean temp[][] = new boolean[alive.length][alive[0].length];
        ...
        for (int i = 0; i < temp.length; i++) {
            for (int j = 0; j < temp[i].length; j++) {
                ...
                if (temp[i][j]) { // If alive
                    if (!(numberOfNeighbors == 2 || numberOfNeighbors == 3)) {
                        temp[i][j] = false; // It is dead
                    }
                }
                else { // If dead
                    if (numberOfNeighbors == 3) {
                        temp[i][j] = true; // It is alive
                    }
                }
            }
        }
        //Copying temp into alive
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src == quitButton) { // End the program.
            ...
            for(int i = 0; i < alive.length; i++) {
                for (int j = 0; j < alive[i].length; j++) {
                    //About 33% of cells are alive for better visual display
                    alive[i][j] = (int)(3*Math.random()) == 0 ? true : false;
                }
            }
            showBoard();
        }
